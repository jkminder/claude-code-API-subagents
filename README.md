...
**One-shot runs:** `claude-api run <slug> "<task>"` — the default way to spawn a worker. Wraps `-p --output-format json` with the session worker dir, prints the worker's answer on stdout (footer with cost + resume handle on stderr), exits nonzero with diagnostics on failure, and logs cost automatically. One background Bash call from the agent's side — same spawn effort as the built-in Task tool, but billed to the API key (worker startup latency and up-front permission grants still differ; for quick in-repo lookups the built-in Explore agent remains the faster tool).
